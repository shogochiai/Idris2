...
// comma in their name (e.g. "…balancedObject,go#p0"). A comma separator collided
// with those commas, so the collector split one id into two and dropped the
// "…,go#pN" tail — those where-helper paths could NEVER join and showed as
// permanently-missing. ';' never appears inside an Idris path-id.
