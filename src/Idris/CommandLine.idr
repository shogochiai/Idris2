...
data CLOpt
  ...
  DumpPathsJSON String |
   ||| Record which execution paths run, to the given file
  DumpPathHits String |
...
options = [MkOpt ["--check", "-c"] [] [CheckOnly]
          ...
          MkOpt ["--show-implicits"] [] [ShowImplicits]
           ...
           MkOpt ["--dumppaths-json"] [Required "output file"] (\f => [DumpPathsJSON f])
              Nothing,
           MkOpt ["--dumppaths-hits"] [Required "output file"] (\f => [DumpPathHits f])
              Nothing, -- record which execution paths run -- dump canonical intrafunction paths as structured JSON
